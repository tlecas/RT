...
static int		ft_fill_properties(t_para *para, char *str)
{
	...
	if (!(ft_strncmp(str, "\tambient: ", 10)))
		para->mat.ambient = ft_atof(tmp = ft_strrcpy(str, 10));
	else if (!(ft_strncmp(str, "\tcolor: ", 8)))
	{
		errno = 0;
		para->color = strtol(tmp = ft_strrcpy(str, 8), &test, 16);
		...
		para->mat.diffuse = ft_atof(tmp = ft_strrcpy(str, 10));
	else if (!(ft_strncmp(str, "\tspecular: ", 11)))
		para->mat.specular = ft_atof(tmp = ft_strrcpy(str, 11));
	else if (!(ft_strncmp(str, "\treflection: ", 13)))
		para->mat.reflection = ft_atof(tmp = ft_strrcpy(str, 13));
	else if (!(ft_strncmp(str, "\trefraction: ", 13)))
		para->mat.refraction = ft_atof(tmp = ft_strrcpy(str, 13));
	else
		return (0);
	free(tmp);
	return (1);
...
static int		ft_fill_coords(t_para *para, char *str)
{
	...
	if (!(ft_strncmp(str, "\tx: ", 4)))
		para->pos.x = ft_atof(tmp = ft_strrcpy(str, 4));
	else if (!(ft_strncmp(str, "\ty: ", 4)))
		para->pos.y = ft_atof(tmp = ft_strrcpy(str, 4));
	else if (!(ft_strncmp(str, "\tz: ", 4)))
		para->pos.z = ft_atof(tmp = ft_strrcpy(str, 4));
	else if (!(ft_strncmp(str, "\tnormalX: ", 10)))
		para->normal.x = ft_atof(tmp = ft_strrcpy(str, 10));
	else if (!(ft_strncmp(str, "\tnormalY: ", 10)))
		para->normal.y = ft_atof(tmp = ft_strrcpy(str, 10));
	else if (!(ft_strncmp(str, "\tnormalZ: ", 10)))
		para->normal.z = ft_atof(tmp = ft_strrcpy(str, 10));
	else if (!(ft_strncmp(str, "\tscalar: ", 9)))
		para->k = ft_atof(tmp = ft_strrcpy(str, 9));
	else
		return (0);
	free(tmp);
	return (1);
...
int		ft_parse_para(t_env *e, char **tab)
{
	...
	while (tab[++i])
	{
		...
		{
			++j;
			e->para[j] = ft_init_para(e->para[j]);
			inpara = 1;
		}
